...
if [ "$1" = "-C" ]; then 
    ...
    ./capture_image settings/camera_settings_color.xml; ERR_CODE=$?
elif [ "$1" = "-M" ]; then
    echo "=== Camera will output a greyscale image ==="
    ./capture_image settings/camera_settings_mono.xml; ERR_CODE=$?
elif [ "$1" = "-H" ]; then
    echo "The host has control"
else
    echo "=== Camera will output a colored and greyscale image ==="
    ./capture_image; ERR_CODE=$?
fi
